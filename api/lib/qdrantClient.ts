...
export class QdrantService {
  ...
  async ensureCollection(
    ...
    try {
      ...
      if (existingCollection) {
        ...
        try {
          ...
          if (currentDimension && currentDimension !== dimension) {
            ...
            console.log(
              `✅ Collection exists: ${collectionName} with correct ${dimension} dimensions`,
            );
          }
        } catch (error) {
          console.warn(
        ...
        console.log(
          `🆕 Creating new collection: ${collectionName} with ${dimension} dimensions`,
        );
        await this.client.createCollection(collectionName, {
          vectors: {
            size: dimension,
      ...
      throw new Error(
        `Failed to initialize Qdrant collection for user: ${userId}`,
      );
    }
  }
  ...
  async upsertChunks(
    ...
    try {
      ...
      console.log(
        `🔗 Ensuring Qdrant collection with ${embeddingDimension} dimensions for user: ${userId}`,
      );
    ...
    try {
      ...
      try {
        ...
        console.log(`📋 Collection info:`, {
          ...
          config: collectionInfo.config?.params?.vectors,
        });
      } catch (collectionError) {
        console.error(
          `❌ Collection not found or error accessing: ${collectionName}`,
          collectionError,
        );
        throw new Error(
          `Collection ${collectionName} not found or inaccessible. Make sure documents are processed and stored.`,
        );
      }
    ...
    try {
      ...
      for (const collectionName of collections) {
        try {
          ...
          console.log(
            `Successfully deleted chunks for document: ${documentId} from ${collectionName}`,
          );
        } catch (error) {
          // Collection might not exist, continue with next one
          console.warn(
            `Could not delete from collection ${collectionName}:`,
            error,
          );
        }
      }
  ...
  async listUserCollections(userId: string): Promise<string[]> {
    try {
      ...
      const userCollections = collections.collections
        .map((c) => c.name)
        .filter((name) =>
          name.startsWith(`${this.baseCollectionName}_user_${userId}`)
        );
  ...
  /**
   * Direct search method for debugging
   */
  async directSearch(
...
export const qdrantService = new QdrantService({
  ...
  collectionName: Deno.env.get("QDRANT_COLLECTION") ||
    "insightsphere-documents",
});
