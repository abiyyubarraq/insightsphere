...
import { freeEmbeddingClient } from "../../lib/freeEmbeddingClient.ts";
import { qdrantService, type DocumentChunk } from "../../lib/qdrantClient.ts";
import {
	chunkText,
	chunkPages,
	createChunkId,
...
export async function processDocument(c: Context) {
	...
	try {
		...
		const { project_id, document_id, storage_path }: DocumentProcessRequest = body;
		...
		if (legacyJwtSecret && token === legacyJwtSecret) {
			...
			const adminDocument = await supabaseService.getDocumentAsAdmin(document_id);
			user = { id: adminDocument.user_id, email: "admin@insightsphere.app" };
			console.log(`Processing document ${document_id} for user ${user.id} (admin mode)`);
		} else {
			// Regular user mode - validate JWT token
			user = await supabaseService.getUserFromToken(token);
			console.log(`Processing document ${document_id} for user ${user.id} (user mode)`);
		}
		...
		if (!isAdminMode) {
			const hasProjectAccess = await supabaseService.validateProjectAccess(project_id, user.id);
			if (!hasProjectAccess) {
				return c.json({
					success: false,
		...
		if (!parseResponse.ok) {
			throw new Error(`Parser service failed: ${parseResponse.status} ${parseResponse.statusText}`);
		}
		...
		console.log(`Parsed document: ${parseResult.text.length} characters, ${parseResult.pages?.length || 0} pages`);
		...
		console.log(`Created ${textChunks.length} text chunks from ${pageContents.length} pages`);
		...
		try {
			...
			totalTokens = embeddings.reduce((sum, emb) => sum + emb.usage.total_tokens, 0);
			console.log(`✅ Generated ${embeddings.length} OpenAI embeddings`);
			
		} catch (openaiError) {
			console.warn("⚠️ OpenAI embeddings failed, switching to free alternative:", openaiError);
			
			// Fall back to free embeddings
			console.log("🆓 Using free embedding service...");
			const freeEmbeddings = await freeEmbeddingClient.generateBatchEmbeddings(
				chunkTexts,
				"auto" // Will try HuggingFace first, then hash fallback
			);
			
			// Convert to OpenAI format for compatibility
			embeddings = freeEmbeddings.map(emb => ({
				embedding: emb.embedding,
				usage: emb.usage
			}));
			
			embeddingModel = freeEmbeddings[0]?.model || "free-embedding";
			totalTokens = freeEmbeddings.reduce((sum, emb) => sum + emb.usage.total_tokens, 0);
			console.log(`✅ Generated ${embeddings.length} free embeddings using ${embeddingModel}`);
		}

		console.log(`Generated ${embeddings.length} embeddings using ${embeddingModel}`);
		...
		await qdrantService.upsertChunks(documentChunks, { useProjectCollection: true });
