...
export async function searchDocuments(c: Context) {
	try {
		...
		if (authHeader?.startsWith(`Bearer ${legacySecret}`)) {
			...
			if (!adminUserId) {
				return c.json({ error: "Admin mode requires X-Admin-User-Id header" }, 400);
			}
			user = { id: adminUserId };
		} else {
			// Normal user authentication
			user = await supabaseService.getUserFromToken(authHeader?.replace("Bearer ", "") || "");
			if (!user) {
				return c.json({ error: "Unauthorized" }, 401);
			}

			// Verify user has access to this project
			const hasAccess = await supabaseService.userHasProjectAccess(user.id, project_id);
			if (!hasAccess) {
		...
		try {
			...
			console.log(`✅ Generated OpenAI embedding (${queryEmbedding.length} dimensions)`);
		} catch (error) {
			console.log("⚠️ OpenAI failed, trying Hugging Face...");
			try {
				const embedding = await freeEmbeddingClient.generateEmbedding(query);
				queryEmbedding = embedding.embedding;
				embeddingModel = embedding.model;
				console.log(`✅ Generated ${embeddingModel} embedding (${queryEmbedding.length} dimensions)`);
			} catch (freeError) {
				console.log("⚠️ Hugging Face failed, using hash embedding...");
				const embedding = await freeEmbeddingClient.generateHashEmbedding(query);
				queryEmbedding = embedding.embedding;
				embeddingModel = embedding.model;
				console.log(`✅ Generated ${embeddingModel} embedding (${queryEmbedding.length} dimensions)`);
			}
		...
		console.log(`🎯 Found ${searchResults.length} relevant chunks in project ${project_id}`);
