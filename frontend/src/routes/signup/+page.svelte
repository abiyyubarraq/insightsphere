<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      info =
        'We just sent a confirmation link to your email. Please verify your address before signing in.';
      // Clear form fields
      password = '';
      confirmPassword = '';
    } catch (err: any) {
...
<svelte:head>
  <title>Sign Up - InsightSphere</title>
</svelte:head>
...
<div class="min-h-screen bg-black flex items-center justify-center p-4 relative overflow-hidden">
  ...
  <div class="w-full max-w-md space-y-8 relative z-10">
    ...
    <!-- Info Message -->
    {#if info}
      <div class="rounded-lg bg-primary/10 border border-primary/30 text-primary px-4 py-3 text-sm">
        {info}
      </div>
    {/if}
    ...
    >
      ...
      {/if}
      <span>{loadingSignupWithGoogle ? 'Signing up...' : 'Sign up with Google'}</span>
    </button>
    ...
    <form on:submit|preventDefault={handleSubmit} class="space-y-4">
      ...
      <!-- Continue Button -->
      <button type="submit" disabled={loadingSignupWithEmail} class="btn btn-primary w-full gap-2">
        {#if loadingSignupWithEmail}
          <Loader2 class="w-4 h-4 animate-spin" />
        {:else}
          <ArrowRight class="w-4 h-4" />
        {/if}
        <span>{loadingSignupWithEmail ? 'Creating account...' : 'Continue'}</span>
      </button>
    </form>
